// =====================================================================
// Chopin Nocturne Op.9 No.2 in E♭ Major — Strudel live-coding 改編
// =====================================================================
// 版本：v6（2026-04-26 存檔）
// 來源 MXL：musetrainer/library MusicXML（archive.org）
// 名版參考：Rubinstein 1965 RCA / 4:26 / archive.org
// 渲染檔：file:///Users/danyuchn/code-music/tracks/drafts/chopin-op9-no2-mxl-v6-pitch-fixed.js
//
// === 進度記錄（已完成 vs 待辦）===
//
// ✅ 已完成：
//   1. 從 MXL 解析全 38 bars（含 anacrusis）
//   2. 三層 voicing：melody（top voice）+ inner（C4-F#4 中音域）+ bass（<C4）
//   3. 32 分音符網格解析度（48 slots/bar）— 涵蓋 coda 飛跑句
//   4. Ornament 展開：8 個 Turn / Mordent / Trill 用 music21 realize
//   5. Bar 3 turn 半音化：D5-C5-B♮4-C5（Chopin 風格 chromatic 下鄰音）
//   6. Bar 2 LH 修補：標準 Eb-G-Bb broken chord ×4（取代 MXL 的 Cb 半音色）
//   7. Per-bar arrange() rubato：38 個係數，coda 漸慢（1.3→2.6）
//   8. Voicing 強化：inner gain g*0.4（-27%）、bass gain g*0.55（-30%）
//   9. Bar 24 melody gain -15%（避免數位音源爆音）
//  10. Bar 31 cadenza rubato 1.4 拉空間
//  11. Coda 平滑化：[1.3, 1.45, 1.6, 1.8, 2.1, 2.6] 單調遞增（Gemini 修正）
//  12. **9 個 bass bar 的 B 自然 → B-flat**（b3, b5, b7, b15, b23, b25, b26, b29, b30）
//      原 MXL transcription 把 Bb 拼成 B♮（增三和弦感），全曲修正完
//  13. 人化情感：melody.early(0.005) lead + 三層 random gain ±5-8% + late jitter
//  14. 動態起伏：每 bar gain 跟著樂譜 18 個力度標記
//  15. Larghetto 60 → 50 BPM（寧靜版，接近 Rubinstein）
//
// ❌ 待辦（後面還有沒對齊的地方，下次再修）：
//   - 全曲 timing 跟 Rubinstein 的精確對齊（Rubinstein bar1=6.10s, bar2=6.42s,
//     bar3=5.59s, bar4=6.08s, bar5=6.37s, bar6=6.30s, bar7=5.95s — 我目前 RUBATO
//     是粗略 1.0/1.05 沒有按 Rubinstein 真實時間做）
//   - Anacrusis 過長（目前 1.0 cycle = 7.2s，Rubinstein 0.61s）
//   - Bar 8-37 範圍可能還有 LH B♮ 殘留（v6 只掃了 bass，melody/inner 沒掃）
//   - Bar 32 cadenza 內部 32 分音符等分（應改成拋物線速度模擬手指物理慣性）
//   - 旋律 voicing 還不夠突出（Gemini 反映 sample stack 三層音色太均勻）
//   - 真正的 rubato 微時差還是太機械（LH 跟 RH 對齊太「垂直」）
//   - 音色：digital piano sample 跟真實音樂廳殘響差距大，沒辦法在 Strudel 補
//
// === Gemini 3.1 Pro Preview 盲測結果（v5 → v6）===
//   - v5 vs Rubinstein 盲測：Gemini 正確識別「Alpha 是電腦」
//     主因：左右手垂直對齊、節拍嚴格量化、裝飾音等分機械
//   - v5 LH 被指控「Eb-G-B♮ 增三和弦」(後驗證部分為真，9 個 bar 確實有 B♮)
//   - v6 修完 9 個 bass B♮ 但沒重做 Gemini 比對
//
// === 草稿索引 ===
//   v8 lo-fi remix:           tracks/drafts/duke-need-u-lofi-v8.js（之前的 Duke 練習）
//   原創 C# minor:            tracks/drafts/chopin-style-original-cs-minor.js
//   v3 MXL 解析:              tracks/drafts/chopin-op9-no2-mxl-v3.js
//   v4 rubato:                tracks/drafts/chopin-op9-no2-mxl-v4-rubato.js
//   v5 Gemini 修正:           tracks/drafts/chopin-op9-no2-mxl-v5-gemini-fixed.js
//   **v6 音準修正（本檔）**:  tracks/drafts/chopin-op9-no2-mxl-v6-pitch-fixed.js
// =====================================================================
...
let b3 = note("[bb2@4 f3@4 bb3@4 bb2@4 g3@4 d4@4 c3@4 g3@4 c4@4 a2@4 f#3@4 c4@4]").s("piano").room(0.55).attack(0.04).release(2.0).gain(0.3025).gain(rand.range(0.95, 1.0))
...
let b5 = note("[eb3@4 g3@4 bb3@4 eb3@4 ab3@4 bb3@4 eb3@4 g3@4 bb3@4 d3@4 g3@4 bb3@4]").s("piano").room(0.55).attack(0.04).release(2.0).gain(0.3025).gain(rand.range(0.95, 1.0))
...
let b7 = note("[bb2@4 f3@4 bb3@4 bb2@4 g3@4 d4@4 c3@4 g3@4 c4@4 a2@4 f#3@4 c4@4]").s("piano").room(0.55).attack(0.04).release(2.0).gain(0.3025).gain(rand.range(0.95, 1.0))
...
let b15 = note("[bb2@4 f3@4 bb3@4 bb2@4 g3@4 d4@4 c3@4 g3@4 c4@4 a2@4 f#3@4 c4@4]").s("piano").room(0.55).attack(0.04).release(2.0).gain(0.3025).gain(rand.range(0.95, 1.0))
...
let b23 = note("[bb2@4 f3@4 bb3@4 bb2@4 g3@4 d4@4 c3@4 g3@4 c4@4 a2@4 f#3@4 c4@4]").s("piano").room(0.55).attack(0.04).release(2.0).gain(0.33).gain(rand.range(0.95, 1.0))
...
let b25 = note("[eb2@4 ab3@4 bb3@4 eb3@4 ab3@4 bb3@4 eb2@4 g3@4 bb3@4 eb3@4 g3@4 bb3@4]").s("piano").room(0.55).attack(0.04).release(2.0).gain(0.22).gain(rand.range(0.95, 1.0))
...
let b26 = note("[eb2@4 ab3@4 bb3@4 eb3@4 ab3@4 bb3@4 eb2@4 g3@4 bb3@4 eb3@4 g3@4 bb3@4]").s("piano").room(0.55).attack(0.04).release(2.0).gain(0.22).gain(rand.range(0.95, 1.0))
...
let b29 = note("[ab1@4 eb3@4 ab3@4 bb3@4 ab3@4 eb3@4 eb2@4 g3@4 bb3@4 eb3@4 g3@4 bb3@4]").s("piano").room(0.55).attack(0.04).release(2.0).gain(0.4675).gain(rand.range(0.95, 1.0))
...
let b30 = note("[ab1@4 eb3@4 ab3@4 bb3@4 ab3@4 eb3@4 eb2@4 g3@4 bb3@4 a2@4 g3@4 c4@4]").s("piano").room(0.55).attack(0.04).release(2.0).gain(0.4675).gain(rand.range(0.95, 1.0))
...
let b31 = note("[bb2@4 f3@4 bb3@4 bb2@4 g3@4 d4@4 c3@4 g3@4 c4@4 a2@4 f3@4 c4@4]").s("piano").room(0.55).attack(0.04).release(2.0).gain(0.5225).gain(rand.range(0.95, 1.0))
